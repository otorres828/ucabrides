...
export const EstaEnCola = ({
  access_token,
  children,
  redirectTo = "/cola/curso",
}) => {
  ...
  if (estatus !== null) {
    if (estatus.cola === "true" || estatus.cola === "aprobado") {
      localStorage.setItem("ucabrides_orden_ruta_id", estatus.orden_ruta_id);
...
export const SalioDeCola = ({
  access_token,
  children,
  redirectTo = "/listado/colas",
}) => {
...
export const VerificarCola = ({
  ...
  const orde_ruta_id = localStorage.getItem("ucabrides_orden_ruta_id");
  if (orde_ruta_id === null) {
    console.log("NO PUEDE INGRESAR HASTA QUE TERMINE SU COLA EN CURSO");
    return <Navigate to={redirectTo} />;
  }
